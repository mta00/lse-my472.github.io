...
unemp$date_trans <- as.Date(unemp$date)


# Create a line-plot with unemployment and date
p <- ggplot(unemp, aes(x = date_trans, y = unemp)) + 
  geom_line() + 
  geom_rect(data=pres, aes(NULL,NULL,xmin=start,xmax=end,fill=party),
            ymin=0,ymax=0.11,colour="white", size=0.5, alpha=0.2) +
  scale_fill_manual(values=c("R" = "red", "D" = "blue")) +
  theme(axis.title.x = element_blank()) +
  xlim(0,max(unemp$date_trans))+
  ylim(0,0.11) +
  theme(legend.position="top") +
  geom_vline(xintercept = pres$end,linetype=4, colour="black")
